...
module.exports = async function handler(req, res) {
  ...
  // Paid API stays disabled until deployment protection and credentials are configured.
  if (process.env.VERCEL && process.env.STORYBOARD_AI_ENABLED !== 'true') return send(res, 503, { error: 'AI chưa được bật cho bản triển khai này.' });
  try {
    let body = req.body;
    if (body === undefined) {
      let raw = '';
